...
EXTENSION_DATA_WAIT = 25          # Increased to ensure extensions have adequate time to load properly
...
class AmazonExtractor:
    ...
    async def _extract_all_data(self, page: Page, asin: str) -> Dict[str, Any]:
        ...
        try:
            ...
            rank_selectors = [
                ...
                "th:has-text('Best Sellers Rank') + td", # More direct for some table structures  
                "xpath=//*[contains(text(),'Best Sellers Rank')]/following-sibling::td", # XPath example
                "xpath=//*[contains(text(),'Best Sellers Rank')]/ancestor::li/span[not(contains(@class,'a-text-bold'))]" # XPath for list items
            ] 
    ...
    async def _extract_selleramp_data(self, page: Page, get_html_content_func, asin: str) -> Dict[str, Any]:
        ...
        # Wait for SellerAmp panel using standardized selectors from triage module
        try:
            await page.wait_for_selector("div#quick-info-body, div.qi-wrapper, div.qi-container, div.qi-row", timeout=5000)
            log.info("SellerAmp quick-info panel detected")
        except Exception as e:
            log.warning(f"SellerAmp quick-info panel not detected: {e}")
        ...
        try:
            ...
            if qi_cost_input:
                # Input a test cost value to trigger calculations
                test_cost = "5.00"  # Use a reasonable test value
                log.info(f"Inputting test cost value: {test_cost}")
                
                # Clear and fill the input field
                await qi_cost_input.clear()
                await qi_cost_input.fill(test_cost)
                
                # Trigger calculation by pressing Enter or Tab
                await qi_cost_input.press("Enter")
                
                # Wait for calculations to complete
                await page.wait_for_timeout(3000)  # 3 second wait for calculations
                log.info("Waiting for SellerAmp calculations to complete...")
