...
class FixedAmazonExtractor(AmazonExtractor):
    ...
    async def search_by_title_using_search_bar(self, title: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Parse search results - extract first few results
            ...
            for element in search_result_elements[:10]:  # More results for title search
                ...
                if asin and len(asin) == 10:  # Valid ASIN format
                    ...
                    result_title = await title_element.inner_text() if title_element else "Unknown Title"
    ...
    async def search_by_ean_and_extract_data(self, ean: str, supplier_product_title: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use page query_selector_all for element selection
            search_result_elements = await page.query_selector_all("div.s-search-results > div[data-asin]")
            
            for element in search_result_elements[:10]:  # Check first 10 results
                asin = await element.get_attribute("data-asin")
                if asin and len(asin) == 10:  # Valid ASIN format
                    # --- Start of new sponsored detection logic (Iteration 2) ---
                    is_sponsored = False
                    
                    # Log the outer HTML of the element for debugging for the first few items
                    # if i < 3: # Log only for the first 3 elements to avoid excessive logging
                    #     element_html_debug = await element.evaluate("element => element.outerHTML")
                    #     log.debug(f"ASIN {asin} HTML: {element_html_debug[:1000]}") # Log more HTML
                    ...
                    if is_sponsored:
                        ...
                        continue
                    # --- End of new sponsored detection logic (Iteration 2) ---
                    
                    # Process organic result:
                    title_element = await element.query_selector("h2 a span") # This seems okay for title
                    ...
                    log.info(f"Found organic result: ASIN {asin} - {title.strip()[:50]}...")
