...
class PassiveExtractionWorkflow:
    ...
    async def _get_ai_suggested_categories_enhanced(
        ...
        if ai_disabled:
            ...
            return await self._dynamic_category_fallback(supplier_url)
    ...
    async def _ai_fallback_ladder(self, category_config: dict, supplier_name: str, formatted: str, memory_context: str, initial_mode: str) -> dict:
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                # Make AI API call with current parameters
                ai_result = await self._call_openai_with_params(prompt, model, temperature)
                
                # Validate the result structure
                if self._result_valid(ai_result):
                    # URL head-check validator - catches hallucinations
                    all_urls = ai_result["top_3_urls"] + ai_result.get("secondary_urls", [])
                    if await self._urls_exist(all_urls):
                        log.info(f"✅ AI FALLBACK SUCCESS on attempt {attempt + 1} with {prompt_mode} mode")
                        return ai_result
                    else:
                        log.warning(f"🔍 URL validation failed - URLs don't exist, retrying with next tier")
                    
                log.warning(f"⚠️ AI result invalid on attempt {attempt + 1} - retrying with next tier")
                
            except Exception as e:
                log.error(f"❌ AI call failed on attempt {attempt + 1}: {e}")
            
            # Clear cache between retries if configured
            if clear_cache and attempt < max_retries:
                await self._invalidate_ai_cache_entry(supplier_name)
        
        # Final resort: dynamic category fallback
        log.error("🚨 ALL AI FALLBACKS FAILED - falling back to dynamic category discovery")
...
**Focus on arbitrage profit margins. Clearance categories = maximum profit opportunity.**"""
    ...
    def _optimize_category_urls(self, urls: list, price_range: str = "low") -> list:
        ...
        # Get max_products_per_category from config
        system_config = self.config.get("system", {})
        max_products_per_category = system_config.get("max_products_per_category", 64)
        
        for url in urls:
            # Base parameters for better product retrieval (generic, no price filtering)
            base_params = f"product_list_limit={max_products_per_category}&product_list_order=price&product_list_dir=asc"
    ...
    async def run(self, supplier_url: str = DEFAULT_SUPPLIER_URL,
        ...
        # Enforce max_analyzed_products limit from system config
        system_config = self.config.get("system", {})
        max_analyzed_products = system_config.get("max_analyzed_products", 0)
        
        if max_analyzed_products > 0 and len(products_to_analyze) > max_analyzed_products:
            products_to_analyze = products_to_analyze[:max_analyzed_products]
            log.info(f"🔧 LIMITED to {max_analyzed_products} products per config (max_analyzed_products)")
    ...
    async def _scrape_single_category(self, category_url: str, supplier_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            system_config = self.config.get("system", {})
            max_products_per_category = system_config.get("max_products_per_category", 50)
