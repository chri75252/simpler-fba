...
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "sk-Avcy8PnI817tyJJT4-qaZN7tDu2f2N8MhlpQgOysjET3BlbkFJx0RXEBfMi6KS_EU1QNSLmWD3pKl54m1bP6PuLHzvgA")
OPENAI_MODEL_NAME = os.getenv("OPENAI_MODEL", "gpt-4.1-mini-2025-04-14")
...
class FixedAmazonExtractor(AmazonExtractor):
    ...
    async def _extract_title_from_element(self, element, asin: str) -> str:
        ...
        # Fallback: try to get any text content from h2 or title-containing elements
        try:
            fallback_element = await element.query_selector("h2, [data-cy*='title'], [class*='title']")
            if fallback_element:
                fallback_text = await fallback_element.inner_text()
                if fallback_text and fallback_text.strip():
                    log.debug(f"ASIN {asin} title extracted using fallback: {fallback_text.strip()[:50]}...")
                    return fallback_text.strip()
        except Exception as e:
            log.debug(f"Fallback title extraction failed for ASIN {asin}: {e}")
    ...
    async def search_by_ean_and_extract_data(self, ean: str, supplier_product_title: str) -> Dict[str, Any]:
        ...
        Relies on base AmazonExtractor's search_by_title (adapted for EAN) and extract_data.
        """
        if not self.browser or not self.browser.is_connected():
            await self.connect()
        ...
        # FIXED: Use proper search bar interaction instead of URL building
        # Get or create a page to work with
        all_pages = self.browser.contexts[0].pages if self.browser.contexts and len(self.browser.contexts) > 0 else []
        if all_pages:
            page = all_pages[0]
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    await page.wait_for_selector("div[data-asin]", timeout=5000)
            
            # Parse search results - extract and filter results using more robust sponsored detection
            organic_results = []
            ...
            if not search_result_elements:
                ...
                search_results_data = {"error": "no_elements_found"}
            else:
                log.info(f"Processing {len(search_result_elements)} search result elements for EAN {ean}")
                
                for i, element in enumerate(search_result_elements[:15]):  # Check more results
                    asin = await element.get_attribute("data-asin")
                    ...
                    # --- Start of new sponsored detection logic (Iteration 3) ---
                    is_sponsored = False
                    ...
                    if i < 3:
                        try:
                            ...
                            log.debug(f"ASIN {asin} HTML structure: {element_html_debug[:500]}...")
                        except Exception:
                            pass
                    ...
                    try:
                        ...
                        if await sponsored_badge_locator.count() > 0: 
                            is_sponsored = True
                            log.info(f"ASIN {asin} detected as sponsored by visible 'Sponsored' text badge.")
                    ...
                    if not is_sponsored:
                        try:
                            if await element.locator('[aria-label="Sponsored"]:visible').count() > 0:
                                is_sponsored = True
                                log.info(f"ASIN {asin} detected as sponsored by aria-label.")
                    ...
                    if not is_sponsored:
                        try:
                            if await element.evaluate("el => el.getAttribute('data-component-type') === 'sp-sponsored-result' || el.getAttribute('data-ad-marker') === 'true' || (el.querySelector('div[data-component-type=\\'sp-sponsored-result\\']') !== null)"):
                                is_sponsored = True
                                log.info(f"ASIN {asin} detected as sponsored by data-component-type or data-ad-marker.")
                    ...
                    if not is_sponsored:
                        try:
                            ...
                            known_ad_classes = ["AdHolder", "s-widget-sponsored-product", "sponsored-results-padding"] 
                            if any(ad_class in element_classes for ad_class in known_ad_classes):
                                is_sponsored = True
                                log.info(f"ASIN {asin} detected as sponsored by tile class: {element_classes}")
                        except Exception as e_class:
                            log.debug(f"Error checking tile classes for ASIN {asin}: {e_class}")

                    if is_sponsored:
                        log.info(f"Skipping sponsored result: ASIN {asin}")
                        continue
                    # --- End of new sponsored detection logic (Iteration 3) ---
                    
                    # Process organic result with improved title extraction:
    ...
    async def extract_data(self, asin: str) -> Dict[str, Any]:
        ...
        # The 'conceptual EAN extraction' previously here is removed as it should be in the base.
...
class PassiveExtractionWorkflow:
    ...
    async def run(self, supplier_url: str = DEFAULT_SUPPLIER_URL,
                  ...
                  debug_smoke: bool = False) -> List[Dict[str, Any]]:
        ...
        supplier_cache_file = os.path.join(self.cache_dir, f"{supplier_name.replace('.', '_')}_products_cache.json")
        supplier_products = []
        ...
        if not supplier_products:
            ...
            if cache_supplier_data and supplier_products:
                try:
                    with open(supplier_cache_file, 'w', encoding='utf-8') as f: json.dump(supplier_products, f, indent=2, ensure_ascii=False)
                    log.info(f"Cached {len(supplier_products)} supplier products")
                except Exception as e: log.error(f"Error caching supplier products: {e}")
        ...
        log.info(f"Processing up to {max_products_to_process} products.")
        products_to_analyze = price_filtered_products[:max_products_to_process]
        ...
        for i, product_data in enumerate(products_to_analyze):
            ...
            if supplier_ean:
                ...
                if amazon_product_data_from_ean_search and "error" not in amazon_product_data_from_ean_search and amazon_product_data_from_ean_search.get("asin"):
                    asin_to_check = amazon_product_data_from_ean_search.get("asin")
                    amazon_product_data = await self._get_cached_amazon_data_by_asin(asin_to_check) # type: ignore
                    if not amazon_product_data:
                        # If not cached, the data from search_by_ean_and_extract_data is already the detailed extracted data
                        amazon_product_data = amazon_product_data_from_ean_search
                        await self._cache_amazon_data(asin_to_check, amazon_product_data) # type: ignore
                    ...
                    # amazon_product_data remains None
                else:
                    log.warning(f"EAN search for '{supplier_ean}' did not yield a usable ASIN.")
                    # amazon_product_data remains None
            ...
            if not amazon_product_data: # Fallback to title search if EAN search fails or no EAN
                ...
                if not amazon_product_data:
                    ...
                    await self._cache_amazon_data(asin_to_check, amazon_product_data)
        ...
        if profitable_results:
            ...
            try:
                with open(output_path, "w", encoding="utf-8") as f: json.dump(profitable_results, f, indent=2, ensure_ascii=False)
                log.info(f"Found {len(profitable_results)} profitable products. Results saved to {output_path}")
            except Exception as e: log.error(f"Error saving results: {e}")
        else:
        ...
        try:
            with open(summary_path, "w", encoding="utf-8") as f: json.dump(self.results_summary, f, indent=2, ensure_ascii=False)
            log.info(f"Workflow summary saved to {summary_path}")
        except Exception as e: log.error(f"Error saving summary: {e}")
    ...
    async def _get_cached_amazon_data_by_asin(self, asin: str) -> Optional[Dict[str, Any]]:
        ...
        if not asin: return None # Added check for None ASIN
        asin_cache_file = os.path.join(self.cache_dir, f"amazon_{asin}.json")
    ...
    async def _cache_amazon_data(self, asin: str, amazon_product_data: Dict[str, Any]):
        """Helper to cache Amazon data."""
        if not asin: return # Added check for None ASIN
        if "error" not in amazon_product_data and amazon_product_data.get("title"):
            ...
            try:
                ...
                log.info(f"Cached Amazon data for ASIN {asin}")
            except Exception as e:
                log.error(f"Error caching Amazon data for ASIN {asin}: {e}")
    ...
    async def _extract_supplier_products(self, supplier_base_url: str, supplier_name: str) -> List[Dict[str, Any]]:
        ...
        # The domain for config loading will be derived from supplier_base_url by the scraper
        supplier_config = self.web_scraper._get_selectors_for_domain(supplier_base_url) # type: ignore
        
        category_paths = supplier_config.get("category_paths", ["/"]) # Default to root if not specified
        use_two_step = supplier_config.get("two_step_extraction", True)
        
        log.info(f"Extracting products from {supplier_name} using {'two-step' if use_two_step else 'single-step'} process. Category paths: {category_paths}")

        for path in category_paths:
            category_url = supplier_base_url.rstrip('/') + path
            ...
            while current_page_num <= max_supplier_pages:
                ...
                if current_page_num > 1 and hasattr(self.web_scraper, 'get_next_page_url'): # Ensure method exists
                    ...
                    # For simplicity in this refactor, we'll assume get_next_page_url can construct
                    # the URL from current_page_num if a pattern is available, or we fetch page 1 first.
                ...
                log.info(f"Found {len(product_elements_soup)} product elements in category {path}, page {current_page_num}")
...
async def run_workflow_main():
    ...
    if OPENAI_API_KEY:
        try:
            ...
            log.info(f"OpenAI client initialized with model {OPENAI_MODEL_NAME}")
        except Exception as e: log.error(f"Failed to initialize OpenAI client: {e}")
    ...
    try:
        log.info(f"Running workflow for {supplier_name} with max {max_products} products. Price range £{min_price}-£{MAX_PRICE}.")
        ...
        if results: log.info(f"Workflow completed. Found {len(results)} potentially profitable products.")
        else: log.info("Workflow completed. No profitable products found.")
    except Exception as e:
