...
class PassiveExtractionWorkflow:
    ...
    def _record_ai_decision(self, hist: dict, ai_response: dict):
        ...
        hist.setdefault("ai_decision_history", []).append({
            ...
            "url_pattern_confidence": ai_response.get("url_pattern_confidence", {})
        })
...
**Focus on arbitrage profit margins. Clearance categories = maximum profit opportunity.**"""
    ...
    def _optimize_category_urls(self, urls: list, price_range: str = "low", max_products: Optional[int] = None) -> list:
        ...
        for url in urls:
            # Base parameters for better product retrieval - always use 64 (max products per page)
            # max_products_per_category controls how many products we scrape in total, not per page
    ...
    async def _hierarchical_category_selection(self, supplier_url, supplier_name, max_products_per_category: int = 0):
    ...
    async def run(self, supplier_url: str = DEFAULT_SUPPLIER_URL,
        ...
        if should_continue_with_ai and hasattr(self, 'ai_client') and self.ai_client:
            ...
            try:
                ...
                new_category_urls = await self._hierarchical_category_selection(supplier_url, supplier_name, max_products_per_category)
    ...
    async def _extract_supplier_products(
        ...
        if use_ai_progression:
            ...
            try:
                category_urls_to_process = await self._hierarchical_category_selection(supplier_base_url, supplier_name, max_products_per_category)
