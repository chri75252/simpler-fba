...
async def perform_zero_token_triage(
    ...
    try:
        ...
        # Enhanced SellerAmp detection with comprehensive polling
        timeout = 45
        poll = 1
        elapsed = 0
        selleramp_detected = False
        
        log.info(f"Enhanced SellerAmp detection - polling for up to {timeout}s...")
        
        while elapsed < timeout and not selleramp_detected:
            try:
                # Check multiple SellerAmp indicators comprehensively
                panel_selectors = [
                    "div#quick-info-body",
                    "div.qi-wrapper", 
                    "div.qi-container",
                    ...
                    if element:
                        ...
                        try:
                            if await element.is_visible():
                                log.info(f"SellerAmp detected via '{selector}' after {elapsed}s")
                ...
                if not selleramp_detected:
                    ...
                    if elapsed % 10 == 0:  # Log every 10 seconds
                        log.info(f"Still waiting for SellerAmp... {elapsed}s elapsed")
        ...
        if not selleramp_detected:
            log.warning(f"SellerAmp extension not detected after {timeout}s timeout")
        else:
            # Additional wait for extension to fully initialize
        ...
        try:
            ...
            qi_cost_input = None
            log.info("Triage: Searching for qi_cost input field...")
            ...
            for sel in SELLERAMP_SELECTORS["qi_cost_input"].split(','):
                ...
                try:
                    ...
                    log.debug(f"Triage: Selector '{sel}' failed: {e}")
                    continue
            
            if qi_cost_input:
                # Enhanced input workflow with better calculation triggering
                test_cost = str(landed_cost) if landed_cost > 0 else "5.00"
                log.info(f"Triage: Starting enhanced input workflow with cost: {test_cost}")
                ...
                log.info(f"Triage: Typed cost value: {test_cost}")
                ...
                for strategy in trigger_strategies:
                    try:
                        if strategy["action"] == "press":
                            ...
                            log.info(f"Triage: Pressed {strategy['key']} to trigger calculation")
                        elif strategy["action"] == "blur":
                            await qi_cost_input.blur()
                            log.info("Triage: Triggered blur event to ensure calculation")
                        
                        await asyncio.sleep(strategy["wait"])
                        
                        # Check if calculations have started by looking for change in ROI/profit fields
                        roi_check = await _safe_extract_text_from_page(page, "roi", SELLERAMP_SELECTORS)
                        profit_check = await _safe_extract_text_from_page(page, "profit", SELLERAMP_SELECTORS)
                        
                        if roi_check or profit_check:
                            log.info(f"Triage: Calculations detected after {strategy['action']} trigger")
                            input_workflow_success = True
                            break
                            
                    except Exception as e:
                        log.warning(f"Triage: Trigger strategy {strategy['action']} failed: {e}")
                        continue
                
                # Final wait for calculations to stabilize
                if input_workflow_success:
                    log.info("Triage: Waiting additional 3s for calculations to stabilize...")
                    ...
                    if dynamic_roi_txt:
                        log.info(f"Triage: Enhanced dynamic ROI extracted: {dynamic_roi_txt}")
                    if dynamic_profit_txt:
                        log.info(f"Triage: Enhanced dynamic profit extracted: {dynamic_profit_txt}")
                else:
                    log.warning("Triage: Input workflow completed but calculations not detected")
            ...
            log.error(f"Triage: Error during enhanced input workflow: {e}")
            # Fallback to static extraction
            dynamic_roi_txt = await _safe_extract_text_from_page(page, "roi", SELLERAMP_SELECTORS)
            dynamic_profit_txt = await _safe_extract_text_from_page(page, "profit", SELLERAMP_SELECTORS)
