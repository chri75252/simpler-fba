...
class PassiveExtractionWorkflow:
    def __init__(self, config_loader: SystemConfigLoader, workflow_config: dict, browser_manager: BrowserManager):
        ...
        # Initialize linking map as a list for detailed format (array of objects)
        self.linking_map = []
            ...
            for batch_num in range(total_batches):
                ...
                for i, product_data in enumerate(batch_products):
                    ...
                    if (supplier_ean or product_data.get("title")) and asin and asin != "NO_ASIN":
                        ...
                        # CRITICAL FIX: Use list append for detailed format
    ...
    def _load_linking_map(self, supplier_name: str) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(linking_map_path):
            try:
                ...
                # Handle both formats: new list format and legacy dict format
                if isinstance(raw_data, list):
                    # New detailed format: [{"supplier_ean": "123", "amazon_asin": "ABC", ...}]
                    linking_map = raw_data
                    self.log.info(f"✅ Loaded linking map (list format) from {linking_map_path} with {len(linking_map)} entries")
                elif isinstance(raw_data, dict):
                    # Legacy simple format: {"EAN": "ASIN", "EAN2": "ASIN2"}
                    linking_map = []
                    for ean, asin in raw_data.items():
                        linking_entry = {
                            ...
                            "supplier_price": None,
                            "amazon_price": None,
                            "match_method": "legacy_import",
                            "confidence": "unknown",
                            "created_at": datetime.now().isoformat(),
                            "supplier_url": ""
                        }
                        linking_map.append(linking_entry)
                    self.log.info(f"✅ Converted linking map from dict format to list format from {linking_map_path} with {len(linking_map)} entries")
                else:
...
Return ONLY valid JSON, no additional text."""
    ...
    def _save_final_report(self, profitable_results: List[Dict[str, Any]], supplier_name: str):
        """Save the final report of profitable products to a CSV file using FBA_Financial_calculator."""
        ...
        if not profitable_results:
            self.log.warning("⚠️ CRITICAL: No profitable products found in this run - no report will be generated!")
            return
            
        # Enhanced error logging for final report generation
        try:
            # Import FBA_Financial_calculator for CSV generation
            from tools.FBA_Financial_calculator import run_calculations
            
            # Use FBA_Financial_calculator to generate comprehensive CSV report
            self.log.info(f"🔍 DEBUG: Calling FBA_Financial_calculator.run_calculations for supplier: {supplier_name}")
            
            # Call the financial calculator to generate the CSV report
            results = run_calculations(
                supplier_name=supplier_name,
                supplier_cache_path=None,  # Will use default supplier-specific path
                output_dir=None,  # Will use default supplier-specific path
                amazon_scrape_dir=None  # Will use default path
            )
            
            # Log the results
            if results and 'file_path' in results:
                self.log.info(f"✅ Financial report generated successfully: {results['file_path']}")
                
                # Verify file was created
                if os.path.exists(results['file_path']):
                    file_size = os.path.getsize(results['file_path'])
                    self.log.info(f"✅ CSV report saved to {results['file_path']} (size: {file_size} bytes)")
                    
                    # Log statistics if available
                    if 'stats' in results:
                        stats = results['stats']
                        self.log.info(f"📊 Report statistics: {stats.get('processed', 0)} processed, {stats.get('found_matches', 0)} found matches, {stats.get('profitable', 0)} profitable")
                else:
                    self.log.error(f"❌ CRITICAL: CSV report file was not created at {results['file_path']}")
            else:
                self.log.error(f"❌ CRITICAL: FBA_Financial_calculator did not return expected results: {results}")
                
        except Exception as e:
            self.log.error(f"❌ CRITICAL: Error generating financial report: {e}", exc_info=True)
    ...
    def _load_linking_map(self, supplier_name: str) -> List[Dict[str, Any]]:
        """Load linking map from supplier-specific JSON file"""
        # Use path_manager for proper path handling and supplier name sanitization
        safe_supplier_name = supplier_name.replace(".", "_").replace("/", "_")
        linking_map_path = path_manager.get_output_path("FBA_ANALYSIS", "linking_maps", safe_supplier_name, "linking_map.json")
            ...
            for i, product_data in enumerate(batch_products):
                ...
                if amazon_data:
                    ...
                    if supplier_ean and amazon_asin:
                        ...
                        # Create detailed linking entry for array format
                        linking_entry = {
                            "supplier_ean": supplier_ean,
                            ...
                            "supplier_price": product_data.get("price"),
                            "amazon_price": amazon_data.get("current_price"),
                            "match_method": "EAN_search",  # Default for chunk processing
                            "confidence": "medium",
                            "created_at": datetime.now().isoformat(),
                            "supplier_url": product_data.get("url")
                        }
