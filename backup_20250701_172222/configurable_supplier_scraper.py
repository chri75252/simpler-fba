...
class ConfigurableSupplierScraper:
    ...
    def __init__(self, ai_client: Optional[Any] = None, openai_model_name: str = OPENAI_MODEL_NAME_DEFAULT, 
        ...
        # AI client for fallback extraction
    ...
    async def scrape_products_from_url(self, url: str, max_products: int = 50) -> List[Dict[str, Any]]:
        ...
        for i, element in enumerate(product_elements[:max_products]):
            try:
                ...
                # Extract availability status (basic detection for category pages)
                availability = await self.extract_availability_category(element, element_html, url)
                out_of_stock = await self.extract_out_of_stock_category(element, element_html, url)
                
                # Selector validation loop - retry with vision if title/price missing
                if not title or not price:
                    product = await self._retry_with_vision_selectors(element, element_html, url, base_url, i+1)
                else:
                    product = {
                        'title': title,
                        'price': price,
                ...
                if product and product.get('title') and product.get('price'):
                    # Optionally filter out out-of-stock products (can be configured)
                    if product.get('out_of_stock', False):
                        log.info(f"Product {i+1} is out of stock: {product['title']} - including anyway for completeness")
                    
                    products.append(product)
                    availability_info = f" (Availability: {product.get('availability', 'Unknown')})" if product.get('availability') else ""
                    log.debug(f"Extracted product {i+1}: {product['title']} - {product['price']}{availability_info}")
                else:
                    log.warning(f"Skipping product {i+1} - missing title or price after retry")
